import 'package:flutter/cupertino.dart';

class Style{
  static const Color primaryColor=Color.fromRGBO(0, 44, 84, 1);
  static const Color secondaryColor=Color.fromRGBO(255, 167, 64, 1);
  static const Color backgroundColor=Color.fromRGBO(255, 255, 255, 1);
  static const Color green=Color.fromRGBO(51, 201, 38, 1);
  static const Color red=Color.fromRGBO(201, 38, 38, 1);
  static const Color blue=Color.fromRGBO(20, 207, 207, 1);
  static const Color shadowColor=Color.fromRGBO(0, 0, 0, .25);
  static const Color grey=Color.fromRGBO(97, 97, 97,1);
}